 ...
 * Node process running the package's MCP entry (`bin/mcp.js`); the spec
 * path is passed through the environment so the generated config is a
 * plain, deterministic JSON document with no embedded secrets. No state
 * path is embedded — the server resolves the state from its own cwd
 ...
 * When installed, `@skillstate/mcp` lives at the same depth as the
 * package's own `dist/` (or `src/`), so `../../../bin/mcp.js` from the
 * compiled module resolves to the root `bin/mcp.js`. Overridable via
 * {@link McpConfigOptions.launcherPath}.
 */
