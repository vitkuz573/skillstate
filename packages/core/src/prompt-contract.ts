...
export function skillMdBody(options: SkillMdBodyOptions): string {
  ...
  return [
    ...
    `The skillstate ${options.hostLabel} hooks:`,
    '',
    `- the CURRENT state is ${options.injectionPhrase} on every prompt submit`,
    '  (`UserPromptSubmit`) and is re-injected after compaction',
    ...
    '   your last look), and/or emit a fenced JSON block with exactly two',
